    ...
    stages {
        ...
        stage('SAST Upload') {
            steps {
                ...
                sh 'java -jar veracode-wrapper.jar -vid ${VeracodeID} -vkey ${VeracodeKey} -action uploadandscan -appname ${VeracodeProfile} -createprofile true  -version $(date +%H%M%s%d%m%y) -filepath ${CaminhoPacote} -deleteincompletescan 1'
            }
        }
        stage('Pipeline Scan') {
            steps {
